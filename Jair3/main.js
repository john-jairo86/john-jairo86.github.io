...
// Ocultar todos los li al inicio
...
btn.addEventListener("click", () => {
    ...
    if (!fieldset.hidden) {
        ...
        items.forEach(li => li.style.display = "none"); // ocultar todo siempre al abrir
        search.focus();
    }
});

// Filtrar opciones solo cuando escriben
...
// Seleccionar opción y enviarla al textarea
items.forEach(item => {
    item.addEventListener("click", () => {
        hiddenValue.value = item.dataset.value;

        const nuevoTexto = item.dataset.value || item.textContent.trim();
...
// Filtrar opciones solo cuando escriben
search_tv.addEventListener("keyup", () => {
    const filter = search_tv.value.toLowerCase();
...
items_tv.forEach(item => {
    item.addEventListener("click", () => {
        hiddenValue_tv.value = item.dataset.value;
...
// Cerrar si clic fuera
document.addEventListener("click", (e) => {
    if (!fieldset_tv.contains(e.target) && e.target !== btn_tv) {
        fieldset_tv.hidden = true;
    }
});
...
// TELEFONO
const btn_tl = document.getElementById("btn_Toggle_tl");
const fieldset_tl = document.getElementById("fielDrop_tl");
const search_tl = document.getElementById("searchInput_tl");
const items_tl = document.querySelectorAll("#optionsList_tl li");
const hiddenValue_tl = document.getElementById("selectedValue_tl");

// Ocultar todos los li al inicio
items_tl.forEach(li => li.style.display = "none");

// Mostrar / ocultar fieldset
btn_tl.addEventListener("click", () => {
    fieldset_tl.hidden = !fieldset_tl.hidden;

    if (!fieldset_tl.hidden) {
        search_tl.value = "";
        items_tl.forEach(li => li.style.display = "none");
        search_tl.focus();
    }
...
// Filtrar opciones solo cuando escriben
search_tl.addEventListener("keyup", () => {
    const filter = search_tl.value.toLowerCase();

    items_tl.forEach(item => {
        const text = item.textContent.toLowerCase();
        item.style.display = (filter && text.includes(filter)) ? "" : "none";
    });
});
...
items_tl.forEach(item => {
    item.addEventListener("click", () => {
        hiddenValue_tl.value = item.dataset.value;
        ...
        fieldset_tl.hidden = true;
    });
});

// Cerrar si clic fuera
document.addEventListener("click", (e) => {
    if (!fieldset_tl.contains(e.target) && e.target !== btn_tl) {
        fieldset_tl.hidden = true;
    }
});
