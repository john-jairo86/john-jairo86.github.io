...
`
    ...
    function copiarPortapapeles(data) {
        const texto = `Id: ${data.idCampo}\nTeléfono: ${data.telefono}\nNombre: ${data.nombre}\nRut: ${data.rut}\nCódigo: ${data.codigo_c}\nNodo: ${data.nodo}\nDirección: ${data.direccion}\nOnt: ${data.ont}\nOlt: ${data.olt}\nTarjeta: ${data.tarjeta}\nPuerto: ${data.puerto}\nObservación: ${data.observacion}`;
    ...
    const wizard2 = initWizard({
        ...
        onGenerate: () => {
            ...
            const datos = {
                ...
                rut: document.getElementById("rut").value,
                codigo: document.getElementById("codigo_c").value,
                direccion: document.getElementById("direccion").value,
                ont: document.getElementById("ont").value,
                olt: document.getElementById("olt").value,
                tarjeta: document.getElementById("tarjeta").value,
...
function abrirModal(tipo) {
    ...
    document.getElementById("box_" + tipo).hidden = false;
    ...
    const modalElement = document.getElementById("modalOpciones");

    modalElement.addEventListener("shown.bs.modal", function handler() {

        setTimeout(() => {
            document.querySelector(".btn-close").blur(); // quita el foco del botón
            document.getElementById("search_" + tipo).focus();
        }, 0);
...
function init() {
    ...
    const datosIniciales = {
        int: [
            ...
            "Sin Internet con luz roja los, reset eléctrico modem se verifica en sistema y vemos perdida de señal luz los, se verifica en twister y vemos que no hay más clientes afectados",
            "Cliente con fallas en servicio de internet con luz roja los, reset eléctrico modem, se verifica en sistema y vemos la ont desconocido, se verifica en historial de morosidad cliente a día en el pago",
            "Sin Internet, reset eléctrico modem, cambio de canal, refresh ip servicio ok",
            "Intermitencia del Internet, reset eléctrico modem, cambio de canal, refresh ip servicio ok",
            "Internet no carga nada, reset eléctrico modem, cambio de canal, refresh ip servicio ok",
            "Conectado sin Internet, reset eléctrico, cambio de canal, refresh ip servicio ok",
            "Cliente indica que módem no enciende, se cambia de tomacorriente, se revisa botón on/off, se verifica y ont está en power off",
        ...
        tv: [
            ...
            "Cliente indica que deco ya no prende, se conecta en otro tomacorriente, se revisa cableado, se revisa botón on/off no retoma",
            "Control no funciona con baterías nuevas, vincular control no retoma se indica sucursal",
            "solicita visita técnica se indica script cliente acepta",
        ],
        tl: [
            "reset eléctrico modem, cambio de canal, refresh ip servicio ok",
            "Cliente con falla en el servicio de Telefonía, indica que el teléfono tiene tono, no puede realizar ni recibir llamadas, reset eléctrico modem, cambio de canal, refresh ip, se revisa niveles dentro del rango, se revisa dentro de la ont estado (IDLE), cable de teléfono, se revisa cableado. Todo está de manera correcta, se realiza prueba, cliente manifiesta que el servicio no retorna",
    ...
    Object.keys(keys).forEach(tipo => {
        ...
        let actual = JSON.parse(localStorage.getItem(keys[tipo]));

        // Si no existe o está vacío → cargar datos iniciales
        if (!actual || actual.length === 0) {
            localStorage.setItem(
                keys[tipo],
                JSON.stringify(datosIniciales[tipo] || [])
            );
        }
    });
}
...
function renderLista(tipo, lista) {
    ...
    lista.forEach(txt => {
        ...
        // 'justify-content-between' separa el texto de los botones
        ...
        span.className = "me-3"; // Añade un margen a la derecha del texto
        span.onclick = () => selectText(txt);

        // Contenedor de botones
        const div = document.createElement("div");
        div.className = "d-flex flex-shrink-0"; // Evita que los botones se deformen

        const btnEdit = document.createElement("button");
        btnEdit.className = "btn btn-warning btn-sm me-1";
        btnEdit.innerHTML = `<i class="fa-solid fa-pen-to-square"></i>`; // Usar innerHTML para asegurar que el emoji cargue bien
        btnEdit.onclick = (e) => { e.stopPropagation(); editarDesdeTexto(tipo, txt); };

        const btnDelete = document.createElement("button");
        btnDelete.className = "btn btn-sm btn-danger";
        btnDelete.innerHTML = `<i class="fa-solid fa-trash-can"></i>`;
        btnDelete.onclick = (e) => { e.stopPropagation(); eliminarDesdeTexto(tipo, txt); };
...
function activarBusqueda(tipo) {
    document.getElementById("search_" + tipo).addEventListener("keyup", function () {
