...
const btn_tv = document.getElementById("btn_Toggle_tv");
        ...
        search_tv.addEventListener("keyup", () => {
            ...
            items_tv.forEach(item => {
                const text = item.textContent.toLowerCase();
                items_tv.style.display = text.includes(filter) ? "" : "none";
            });
        });
        ...
        items_tv.forEach(item => {
            item.addEventListener("click", () => {
                // btn.textContent = item.textContent;         // mostrar selección
                hiddenValue_tv.value = item.dataset.value;     // guardar valor
                fieldset_tv.hidden = true;                     // cerrar fieldset
            });
        });
...
document.querySelectorAll('#optionsList_tv .list-group-item').forEach(item => {
    item.addEventListener('click', function () {
