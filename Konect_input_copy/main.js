...
document.addEventListener("click", async (e) => {
    ...
    //Copiar input y mostrar tipificacion
...
Observación: ${$OBS}`
    ...
    if (e.target.matches("#btn_nombre")) {
        try {
            await navigator.clipboard.writeText(`${$NOMBRE_C}`)
    ...
    //copiar direccion
    if (e.target.matches("#btn_direccion")) {
        try {
            await navigator.clipboard.writeText(`${$DIRECCION_ONT_OLT}`)
    ...
    if (e.target.matches("#btn_id")) {
        try {
            await navigator.clipboard.writeText(`${$ID_LL}`)
    ...
    if (e.target.matches("#btn_nodo")) {
        try {
            await navigator.clipboard.writeText(`${$NODO}`)
